...
const uri = "mongodb+srv://likhithnv45:7ztuyRlzL5EpTmtm@cluster0.zlwfkzp.mongodb.net/?retryWrites=true&w=majority";
...
async function run() {
    try {
          ...
          const postColletion = client.db('database').collection('posts') // this is post collection
          const userColletion = client.db('database').collection('users') // this is user collection
  
          // get
          app.get('/post', async (req,res) => {
              const post = (await postColletion.find().toArray()).reverse();
              res.send(post);
          })
          ...
          app.get('/loggedInUser', async (req,res) => {
              ...
              const user = await userColletion.find({email:email}).toArray();
              res.send(user);
          })
          app.get('/userPost', async (req,res) => {
              const email = req.query.email;
              const post = (await postColletion.find({email: email}).toArray()).reverse();
              res.send(post);
          })
          ...
          app.post('/post', async (req, res) => {
              const post = req.body;
              const result = await postColletion.insertOne(post);
              res.send(result);
          })
